...
public class StaticSeargeAdapter {
    ...
    public StaticSeargeAdapter(Class<?> targetClass, Object targetInstance) {
    ...
    public List<Method> findFunc(boolean debug, Object... args) {
        ...
        preFuncs.stream()
                ...
                .forEach(func -> {
                    ...
                    if (debug) AsunaMod.logMsg(true, "Parameter count matched: " + func.getName());
                });
        if (parameterSizeFuncs.size() == 0) {
            if (debug) AsunaMod.logMsg(true, "No matching functions were found!");
            return null;
        }
        if (parameterSizeFuncs.size() == 1) {
            List<Method> l = new ArrayList<>();
            l.add(parameterSizeFuncs.get(0));
            if (debug) AsunaMod.logMsg(true, "Found exactly one function: " + l.get(0).getName());
            return l;
        }
        /* ok so like there's multiple functions in this arraylist so now we have to narrow them down
         * EVEN MORE smh ;-;
         **/
        if (debug) AsunaMod.logMsg(true, "Need to be more specific...");
        ...
        parameterSizeFuncs.forEach(func -> {
            for (int i = 0; i < args.length; i++) {
                ...
                if (debug) AsunaMod.logWarn(true, func.getParameterTypes()[i].getSimpleName()
                        + " vs " + args[i].getClass().getSimpleName() + " = " + flag);
                if (!flag) {
                    return;
                }
            }
            if (debug) AsunaMod.logMsg(true, "Specifically found: " + func.getName());
            theMethods.add(func);
        });
        return theMethods;
    }
    ...
    public List<Object> invoke(List<Method> funcs, Object... args) {
        ...
        for (Method func : funcs) {
            c++;
            AsunaMod.logMsg(false, "Executing " + func.getName() + " @ position " + c);
            func.setAccessible(true);
            if (func.getGenericReturnType().getTypeName().equalsIgnoreCase("void")) {
                try {
                    func.invoke(null, args);
            ...
            try {
                returns.add(func.invoke(null, args));
            } catch (IllegalAccessException | InvocationTargetException e) {
                e.printStackTrace();
            }
            returns.add(null);
