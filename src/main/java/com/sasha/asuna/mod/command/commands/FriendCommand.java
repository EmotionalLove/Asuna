...
public class FriendCommand extends SimpleCommand {
    ...
    public void onCommand() {
        if (this.getArguments() == null || this.getArguments().length != 2) {
            AsunaMod.logErr(false, "Arguments required! Try \"-help command friend\"");
            return;
        }
        switch (this.getArguments()[0].toLowerCase()) {
            case "add":
                if (AsunaMod.FRIEND_MANAGER.isFriended(this.getArguments()[1])) {
                    AsunaMod.logErr(false, "That person is already friended!");
                    return;
                }
                AsunaMod.FRIEND_MANAGER.addFriend(this.getArguments()[1]);
                AsunaMod.logMsg(false, this.getArguments()[1] + " successfully added");

                break;
            case "del":
                if (!AsunaMod.FRIEND_MANAGER.isFriended(this.getArguments()[1])) {
                    AsunaMod.logErr(false, "That person isn't friended!");
                    return;
                }
                AsunaMod.FRIEND_MANAGER.removeFriend(this.getArguments()[1]);
