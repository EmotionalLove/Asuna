...
public class AsunaGuiWindow implements IAsunaGuiElement {
    ...
    private List<IAsunaGuiElement> moduleElements = new ArrayList<>();
    private int x;
    private int length;
    private int y;
    private int width;
    ...
    public AsunaGuiWindow(int x, int y, int length, int width, String title, List<IAsunaGuiElement> moduleElements) {
        this.x = x;
        this.y = y;
        this.length = length;
        this.width = width;
    ...
    public AsunaGuiWindow(int x, int y, int length, int width, int colour, String title, List<IAsunaGuiElement> moduleElements) {
        this.x = x;
        this.y = y;
        this.title = title;
        this.length = length;
    ...
    public AsunaGuiWindow(int x, int y, int length, int width, float colourR, float colourG, float colourB, float colourA, String title, List<IAsunaGuiElement> moduleElements) {
        this.x = x;
        this.y = y;
        this.length = length;
        this.width = width;
    ...
    private void drawTitlebar() {
        ...
        AsunaMod.FONT_MANAGER.segoe_36.drawCenteredString(this.title, (this.x + (this.width / 2)), this.y + 5, 0xffffff, true);
    }

    private void drawRestOfWindow() {
        AsunaMath.drawRect(this.x, (this.y + TITLEBAR_BOUND), this.x + this.width, (this.y + TITLEBAR_BOUND) + this.length, Integer.MIN_VALUE);
    }

    @Override
    public boolean onMouseEngage(int x, int y, int b) {
        for (IAsunaGuiElement moduleElement : this.moduleElements) {
        ...
        if (b == 0) {
            if ((x >= this.x && x <= (this.x + this.width))
                ...
                drag = true;
                AsunaClickGUI.requestFocus(this);
                return true;
            }
        }
        return false;
