...
package com.sasha.asuna.mod.gui.waypointgui;
...
import static com.sasha.asuna.mod.gui.waypointgui.WaypointGUI.windowList;
...
public class WaypointButton {
    ...
    public void draw() {
        ...
        AsunaMath.drawBetterBorderedRect(x + window.getDragX(), y + window.getDragY(), x + 196 + window.getDragX(), y + 22 + window.getDragY(), 0.5F, var1, var2, var3, var4, 1f, 1f, 1f, varia);
        ...
        AsunaMod.FONT_MANAGER.segoe_30.drawStringWithShadow(displayName, (x + 4 + window.getDragX()), (int) y + window.getDragY(), 0xFFFFFF);
        AsunaMod.FONT_MANAGER.segoe_30.drawStringWithShadow("\247" + "7XYZ " + "\247" + "r" + mod.getCoords()[0] + " " + mod.getCoords()[1] + " " + mod.getCoords()[2], (x + 4 + window.getDragX()), (y + window.getDragY()) + 10, 0xFFFFFF);
    }
    ...
    public void mouseClicked(int x, int y, int button) { //TODO: make sure only one button gets toggled even if the windows are overlapping
        if (x >= getX() + window.getDragX() && y >= getY() + window.getDragY() && x <= getX() + 196 + window.getDragX() && y <= getY() + 21 + window.getDragY() && window.isOpen() && button == 0) {
            ...
            mod.toggle();
            try {
                AsunaMod.DATA_MANAGER.saveWaypoint(mod, false);
            } catch (IOException e) {
                e.printStackTrace();
            }
            AsunaMod.minecraft.world.playSound(AsunaMod.minecraft.player.posX, AsunaMod.minecraft.player.posY, AsunaMod.minecraft.player.posZ, SoundEvents.UI_BUTTON_CLICK, SoundCategory.AMBIENT, 1f, 1f, false);
            ...
            WaypointGUI.sendPanelToFront(window);
            AsunaMod.minecraft.world.playSound(AsunaMod.minecraft.player.posX, AsunaMod.minecraft.player.posY, AsunaMod.minecraft.player.posZ, SoundEvents.BLOCK_CHEST_OPEN, SoundCategory.MASTER, 1f, 1f, false);
