...
public class WaypointWindow {
    ...
    public void draw(int x, int y) {
        if (isShown) {
            ...
            AsunaMath.drawBetterBorderedRect(getXAndDrag(), getYAndDrag(), getXAndDrag() + 200, getYAndDrag() + 13 + (isEmpty ? 175 : (isOpen ? 12 + (24 * (buttonList.size()/* + optionButtonList.size()*/) + 0.5F) + (0 + (0 != 0 ? 2.5F : 0)) : 0) + toAdd), 0.5F, 0f, 0.5f, 1f, 1f, 0f, 0f, 0.5f, 0.3f);
            AsunaMod.FONT_MANAGER.segoe_30.drawCenteredString(title, getXAndDrag() + 100, getYAndDrag() + 1, 0xFFFFFF, true);
            ...
            if (isOpen) {
                ...
                if (buttonList.size() == 0) {
                    AsunaMod.FONT_MANAGER.segoe_36.drawCenteredString("Well, you won't find anything useful here.", getXAndDrag() + 100, getYAndDrag() + 75, 0xFFFFFF, true);
                    AsunaMod.FONT_MANAGER.segoe_30.drawCenteredString("Use the -waypoint command to set and delete waypoints.", getXAndDrag() + 100, getYAndDrag() + 85, 0xFFFFFF, true);
                    isEmpty = true;
                } else {
                    isEmpty = false;
                }
    ...
    public void loadButtonsFromCategory(AsunaCategory category) {
        for (Waypoint m : AsunaMod.WAYPOINT_MANAGER.getWaypoints()) {
            addButton(m);
        }
    }
