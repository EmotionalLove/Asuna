...
public abstract class XdolfRender {
        ...
        try {
            ...
            for (Entity e : XdolfMod.minecraft.world.loadedEntityList) {
                if (e instanceof EntityOtherPlayerMP) {
                    i++;
                    double posX = e.lastTickPosX + (e.posX - e.lastTickPosX) - XdolfMod.minecraft.renderManager.renderPosX;
                    double posY = e.lastTickPosY + (e.posY - e.lastTickPosY) - XdolfMod.minecraft.renderManager.renderPosY;
                    double posZ = e.lastTickPosZ + (e.posZ - e.lastTickPosZ) - XdolfMod.minecraft.renderManager.renderPosZ;
                    if (XdolfMod.FRIEND_MANAGER.isFriended(e.getName())) {
                        GL11.glColor3f(0.0f, 2.0F, 1.0F);
                    } else {
                        float factor = XdolfMod.minecraft.player.getDistance(e) / 40F;
                        if (factor > 1)
                            factor = 1;

                        GL11.glColor4f(2 - factor * 2F, factor * 2F, 0, 0.5F);
                    }
                    Vec3d eyes = new Vec3d(0.0D, 0.0D, 1.0D).rotatePitch(-(float) Math.toRadians(XdolfMod.minecraft.player.rotationPitch)).rotateYaw(-(float) Math.toRadians(XdolfMod.minecraft.player.rotationYaw));
                    GL11.glBegin(GL11.GL_LINES);
                    GL11.glVertex3d(eyes.x, XdolfMod.minecraft.player.getEyeHeight() + eyes.y, eyes.z);
    ...
    public static void storageESP(double x, double y, double z, float r, float g, float b) {
        double posX = x - XdolfMod.minecraft.renderManager.renderPosX;
        double posY = y - XdolfMod.minecraft.renderManager.renderPosY;
        double posZ = z - XdolfMod.minecraft.renderManager.renderPosZ;
        GL11.glPushMatrix();
