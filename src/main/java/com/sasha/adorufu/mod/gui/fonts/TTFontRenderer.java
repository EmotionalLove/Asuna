...
package com.sasha.adorufu.mod.gui.fonts;
...
public class TTFontRenderer extends FontRenderer
{
	
	public final Random fontRandom = new Random();
	private final Color[] customColorCodes = new Color[256];
	private final int[] colorCode = new int[32];
	private TTFont font, boldFont, italicFont, boldItalicFont;
	private String colorcodeIdentifiers = "0123456789abcdefklmnor";
	private boolean bidi;
	

	public TTFontRenderer(final Font font, final boolean antiAlias, final int charOffset) {
	...
	{
		int shadow;
		shadow = (color & 16579836) >> 2 | color & -16777216;
		drawString(s, x + 1, y + 1, shadow, true);
		...
		int color, final boolean shadow)
	{
		...
		{
			final String[] parts = text.split(String.valueOf("\247"));
	 ...
	 * Make dis
	 */
	private String toRandom(final TTFont font, final String text)
	{
		String newText = "";
		...
		final int charOffset)
	{
		synchronized(this)
		{
			this.font = new TTFont(font, antiAlias, charOffset);
			boldFont = new TTFont(font.deriveFont(Font.BOLD), antiAlias,
				charOffset);
			italicFont = new TTFont(font.deriveFont(Font.ITALIC), antiAlias,
				charOffset);
			boldItalicFont =
				new TTFont(font.deriveFont(Font.BOLD | Font.ITALIC),
			...
			for(final String word : words)
			{
				...
				else
				{
					...
					currentWord = lastColorCode == -1 ? word + " "
						: String.valueOf("\247") + lastColorCode + word + " ";
				}
			}
			if(!currentWord.equals(""))
				if(getStringWidth(currentWord) < width)
				{
					finalWords.add(lastColorCode == -1 ? currentWord + " "
						: String.valueOf("\247") + lastColorCode + currentWord + " ");
					currentWord = "";
				}else
		...
		for(int i = 0; i < s.toCharArray().length; i++)
		{
			...
			else
			{
				...
				currentWord = lastColorCode == -1 ? String.valueOf(c)
					: String.valueOf("\247") + lastColorCode + String.valueOf(c);
			}
		}
